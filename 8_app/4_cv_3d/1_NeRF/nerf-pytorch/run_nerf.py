...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 判断是否支持cuda
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    # reshape 为[1024,64,4] 4:rgb, alpha
...
def create_nerf(args):
    ...
    #2. 输入网络深度和每层宽度，输入通道数不是5，而是位置编码后的通道数
    #模型初始化
    ...
    #定义lambda通道，将查询传递给network_fn的函数，模型批处理数据
...
def config_parser():
    ...
    parser.add_argument("--datadir", type=str, default='./data/llff/fern', 
                        help='input data directory')        # 输入数据路径

    # training options 训练需要的参数
    parser.add_argument("--netdepth", type=int, default=8, 
                        help='layers in network')       # 网络深度（层数）
    parser.add_argument("--netwidth", type=int, default=256, 
                        help='channels per layer')      # 每层网络的通道数
    parser.add_argument("--netdepth_fine", type=int, default=8, 
                        help='layers in fine network')  # fine网络层数
    parser.add_argument("--netwidth_fine", type=int, default=256, 
                        help='channels per layer in fine network')  # 每层网络的通道数
    ...
    parser.add_argument("--no_batching", action='store_true', 
                        help='only take random rays from 1 image at a time')  # 一次只从一张图像中获取随机光线
    parser.add_argument("--no_reload", action='store_true', 
                        help='do not reload weights from saved ckpt')   # 不从保存的ckpt加载权重
    parser.add_argument("--ft_path", type=str, default=None, 
                        help='specific weights npy file to reload for coarse network') # 为coarse网络加载权重

    # rendering options 渲染相关参数
    parser.add_argument("--N_samples", type=int, default=64, 
                        help='number of coarse samples per ray')  # coarsen网络每条光线采样的点数
    parser.add_argument("--N_importance", type=int, default=0, 
                        help='number of additional fine samples per ray')  # fine网路每条光纤增加的精细
    ...
    parser.add_argument("--multires", type=int, default=10, 
                        help='log2 of max freq for positional encoding (3D location)')  # 多分辨率位置编码最大频率的log2（3D位置）
    parser.add_argument("--multires_views", type=int, default=4,            
                        help='log2 of max freq for positional encoding (2D direction)')  #位置编码的最大频率的log2（2D方向）
    parser.add_argument("--raw_noise_std", type=float, default=0., 
                        help='std dev of noise added to regularize sigma_a output, 1e0 recommended') # 噪声方差

    parser.add_argument("--render_only", action='store_true', 
                        help='do not optimize, reload weights and render out render_poses path')  # 仅渲染，加载权重，不训练
    parser.add_argument("--render_test", action='store_true', 
                        help='render the test set instead of render_poses path')  # 渲染测试集而不是pose路径
    ...
    parser.add_argument("--no_ndc", action='store_true', 
                        help='do not use normalized device coordinates (set for non-forward facing scenes)')# 默认不适用标准化坐标系
    ...
    parser.add_argument("--i_weights", type=int, default=10000, 
                        help='frequency of weight ckpt saving')  # 每隔10000词保存权重
    parser.add_argument("--i_testset", type=int, default=50000, 
                        help='frequency of testset saving')      # 
    parser.add_argument("--i_video",   type=int, default=50000,  
                        help='frequency of render_poses video saving') # 每隔50000次保存视频
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        if not isinstance(i_test, list):
            i_test = [i_test]  # 转成list

        if args.llffhold > 0: #len=3
    ...
    # 创建log路径，保存训练用的参数到args,复制到config参数并保存
    basedir = args.basedir
    expname = args.expname
